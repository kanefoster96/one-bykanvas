...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    return html(res, 200, page({
      heading: 'Turn off site updates?',
      body: 'You will stop getting emails telling you when something new has '
  ...
  try {
    ...
    const { error } = await db.from('profiles')
      .update({ notify_optout: true }).eq('id', userId);
    if (error) throw new Error(error.message);

    console.log('unsubscribe: %s opted out of notifications', userId);
    return html(res, 200, page({
      heading: 'Done',
      body: 'You will not get site update emails from us again. Turn them back '
