...
async function start() {
  ...
  await showFeatures();
  await showMarketing(profile.data);
  await loadTemplates();
 ...
 * here - RLS scopes every row to the owner - but the send itself goes
 * through the server, which checks the plan, builds the unsubscribe links
 * and does the actual sending. Until the admin sets their sending address
 * the composer stays hidden and the panel says it is being set up, so the
...
async function showMarketing(row) {
  var panel = document.getElementById('marketingPanel');
  if (!panel) return;
  ...
  var from = row.campaign_from || '';
  var statusEl = document.getElementById('mkStatus');
  document.getElementById('mkForm').hidden = !from;
  statusEl.textContent = from
    ? 'Included with your plan: email your customers as often as you like, sent from ' + from + '.'
    : 'Included with your plan. We’re setting up your sending address (you@yourbusiness.co.uk) — '
      + 'you’ll get a notification when it’s ready. Build your customer list in the meantime.';
...
async function refreshAudience() {
  var q = await ONE.db.from('audience')
    .select('id, email, name, unsubscribed_at')
    .order('created_at', { ascending: false }).limit(MK_MAX_LIST);
  mkAudience = (!q.error && q.data) || [];

  var activeCount = mkAudience.filter(function (r) { return !r.unsubscribed_at; }).length;
  var goneCount = mkAudience.length - activeCount;
  document.getElementById('mkCount').textContent = mkAudience.length
    ? activeCount + (activeCount === 1 ? ' person' : ' people') + ' on your list'
      + (goneCount ? ' · ' + goneCount + ' unsubscribed' : '')
    : 'No one on your list yet — add your first customers below.';

  var wrap = document.getElementById('mkListWrap');
  var list = document.getElementById('mkList');
  list.textContent = '';
  if (!mkAudience.length) { wrap.hidden = true; return; }

  mkAudience.forEach(function (r) {
    var li = el('li', 'mk-row' + (r.unsubscribed_at ? ' is-gone' : ''));
    li.appendChild(el('span', 'mk-mail', (r.name ? r.name + ' — ' : '') + r.email
      + (r.unsubscribed_at ? ' (unsubscribed)' : '')));
    var rm = el('button', 'linkish mk-rm', 'Remove');
    rm.type = 'button';
    rm.addEventListener('click', async function () {
      rm.disabled = true;
      var q2 = await ONE.db.from('audience').delete().eq('id', r.id);
      if (q2.error) { alert(ONE.friendlyError(q2.error)); rm.disabled = false; return; }
      await refreshAudience();
    });
    li.appendChild(rm);
    list.appendChild(li);
...
/* "jane@x.com, Sam <sam@y.com>" - commas or new lines between people, an
   optional name around the address. Anything that does not contain an email
   is quietly dropped rather than failing the batch. */
function parseRecipients(text) {
  ...
  String(text || '').split(/[\n,;]+/).forEach(function (part) {
    ...
    var name = part.replace(m[1], '').replace(/[<>"']/g, '').trim().replace(/\s+/g, ' ');
    out.push({ email: email, name: name.slice(0, 80) || null });
  });
  return out;
}

document.getElementById('mkAddBtn').addEventListener('click', async function () {
  var note = document.getElementById('mkAddNote');
  var box = document.getElementById('mkAdd');
  var btn = this;

  var people = parseRecipients(box.value);
  if (!people.length) return say(note, 'Type at least one email address.', 'bad');
  if (!document.getElementById('mkConsentBox').checked) {
    return say(note, 'Tick the box to confirm these people are happy to hear from you — marketing email needs their OK.', 'bad');
  }

  var have = {};
  mkAudience.forEach(function (r) { have[r.email] = true; });
  var fresh = people.filter(function (p) {
    if (have[p.email]) return false;
    have[p.email] = true;   // also dedupes within the pasted text itself
    return true;
  });
  if (!fresh.length) return say(note, 'Already on your list.', 'ok');
  if (mkAudience.length + fresh.length > MK_MAX_LIST) {
    return say(note, 'Lists are capped at ' + MK_MAX_LIST + ' for now — email us if yours is bigger.', 'bad');
  }

  btn.disabled = true;
  say(note, 'Adding…');
  var rows = fresh.map(function (p) { return { user_id: user.id, email: p.email, name: p.name }; });
  var q = await ONE.db.from('audience').insert(rows);
  btn.disabled = false;
  if (q.error) return say(note, ONE.friendlyError(q.error), 'bad');

  box.value = '';
  say(note, 'Added ' + fresh.length + (fresh.length === 1 ? ' person.' : ' people.'), 'ok');
  await refreshAudience();
});

/* The image goes to their own folder in a public bucket the moment it is
   picked - an email cannot fetch anything private, so public is the point. */
document.getElementById('mkImage').addEventListener('change', async function (e) {
  var note = document.getElementById('mkImageNote');
  var file = e.target.files && e.target.files[0];
  mkImageUrl = '';
  if (!file) { say(note, ''); return; }
  if (file.size > 3 * 1024 * 1024) {
    e.target.value = '';
    return say(note, 'Keep it under 3 MB — emails load faster that way.', 'bad');
  }
  say(note, 'Uploading…');
  var ext = (file.name.split('.').pop() || 'jpg').toLowerCase().replace(/[^a-z0-9]/g, '') || 'jpg';
  var path = user.id + '/' + Date.now() + '.' + ext;
  var up = await ONE.db.storage.from('campaign-images')
    .upload(path, file, { contentType: file.type });
  if (up.error) { e.target.value = ''; return say(note, ONE.friendlyError(up.error), 'bad'); }
  mkImageUrl = window.ONE_SUPABASE.url + '/storage/v1/object/public/campaign-images/' + path;
  say(note, 'Ready — it’ll sit at the top of the email.', 'ok');
});

document.getElementById('mkLink').addEventListener('input', function () {
  document.getElementById('mkLinkTextField').hidden = !this.value.trim();
});

document.getElementById('mkForm').addEventListener('submit', async function (e) {
  e.preventDefault();
  var note = document.getElementById('mkNote');
  var btn = document.getElementById('mkSendBtn');

  var subject = document.getElementById('mkSubject').value.trim();
  var message = document.getElementById('mkMessage').value.trim();
  var linkUrl = document.getElementById('mkLink').value.trim();
  var linkText = document.getElementById('mkLinkText').value.trim();
  if (!subject) return say(note, 'Give it a subject.', 'bad');
  if (!message) return say(note, 'Write the message.', 'bad');

  var count = mkAudience.filter(function (r) { return !r.unsubscribed_at; }).length;
  if (!count) return say(note, 'Add some customers to your list first.', 'bad');
  if (!confirm('Send this to ' + count + (count === 1 ? ' person' : ' people') + ' now?')) return;
  ...
  try {
    ...
    say(note, 'Sent to ' + data.sent + (data.sent === 1 ? ' customer.' : ' customers.'), 'ok');
    document.getElementById('mkSubject').value = '';
    document.getElementById('mkMessage').value = '';
    document.getElementById('mkLink').value = '';
    document.getElementById('mkLinkText').value = '';
    document.getElementById('mkLinkTextField').hidden = true;
    document.getElementById('mkImage').value = '';
    say(document.getElementById('mkImageNote'), '');
    mkImageUrl = '';
    await refreshCampaigns();
  } catch (err) {
...
async function refreshCampaigns() {
  var q = await ONE.db.from('campaigns')
    .select('id, subject, recipient_count, sent_at')
    .order('sent_at', { ascending: false }).limit(12);
  var rows = (!q.error && q.data) || [];

  var wrap = document.getElementById('mkHistory');
  var list = document.getElementById('mkHistoryList');
  list.textContent = '';
  if (!rows.length) { wrap.hidden = true; return; }
  rows.forEach(function (c) {
    var li = el('li', 'mk-row');
    li.appendChild(el('span', 'mk-mail', '“' + c.subject + '”'));
    li.appendChild(el('span', 'mk-when',
      c.recipient_count + ' sent · ' + new Date(c.sent_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })));
    list.appendChild(li);
  });
  wrap.hidden = false;
}
